...
| Description **quick‑pick chips**, emoji‑tagged and **filtered by mode** — Parents vs Private show different buttons (`DESCRIPTIONS` in `config.js`; a chip can insert a longer phrase via `text`) | one tap, only the relevant options |
